...
public class AncestorService : AncestorServ.AncestorServBase
{
    ...
    )
    {
        ...
        Console.WriteLine($"GetAncestor by user {user.Id} : {user.Username} for anc {ancestor.Id} : {ancestor.BurialPlace} : {ancestor.OwnerId}");
    ...
    )
    {
        ...
        Console.WriteLine($"CreateAncestor by user {user.Id} : {user.Username} : {user.DirectAncestors} for anc {ancestor.Id} : {ancestor.BurialPlace}");
